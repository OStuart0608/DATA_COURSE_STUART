...
iris_subset <- iris
nrow(iris_subset)
rows <- nrow(iris_subset)
even_rows <- seq_len(rows) %% 2

data_mod <- iris_subset[even_rows == 0, ]
print(data_mod)
...
iris
sapply(iris, class)
iris_chr <- transform(iris, Sepal.Length = as.character(Sepal.Length), Sepal.Width = as.character(Sepal.Width), Petal.Length = as.character(Petal.Length), Petal.Width = as.character(Petal.Width), Species = as.character(Species))
